...
def encoding(
    ...
    with torch.no_grad():
        for x, batch_dates in dataloader:
            ...
            # Forward pass and only keep the latent representation
            z, _ = model(x_in)
    ...
    torch.save(
        {"latent": torch.cat(latent_list, dim=0), "dates": dates_list},
        save_dir / f"{split_name}_{date_start}_{date_end}.pt",
    )
...
def aggregate(
    ...
    torch.save(
        torch.cat(latent_list, dim=0), save_dir / f"{split_name}_{date_start}_{date_end}.pt"
    )
